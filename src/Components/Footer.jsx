...
import { initFooter, termsOfUse } from './navBar'

function Footer() {
  return (
    <div style={{backgroundColor: '#000000', height: '90vh'}}>
        <div style={{ color: '#FFFFFF', display: 'flex', flexWrap: 'wrap', padding: '50px 90px', gap: '30px', backgroundColor: '#000000' }}>
          {initFooter.map(x => (
              <div style={{ width: '18%' }}>
                  <p style={{ textAlign: 'center', paddingBottom: '10px', fontSize: '16px', fontWeight: 'bold' }}>{x.title}</p>
                  {x?.list.map(y => <p style={{ textAlign: 'center', fontSize: '14px', marginBottom: '10px', color: '#AAAAAA' }}>{y}</p>)}
              </div>
          ))}
